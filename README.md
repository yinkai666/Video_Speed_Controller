...
- `↑` ：增加音量（每次+10%）
- `↓` ：减小音量（每次-10%）
- `Shift` + `↑` ：音量增强（超过100%，需先将音量调至100%）
- `Shift` + `↓` ：降低音量增强（降至100%即关闭）
...
- 新增音量增强功能：通过 Web Audio API 实现超过 100% 的音量放大，按 `Shift + ↑/↓` 控制
- 首次启用时弹出听力保护提示
- 支持菜单自定义增强上限（默认4倍）和步长（默认0.1）
- 自动检测跨域视频并禁用增强以避免静音

### v1.6.4

- 修复鼠标点击B站全屏按钮后脚本快捷键失效的问题

### v1.6.3

- 修复长按 [ ] 键时事件穿透到网站原生快捷键的问题

### v1.6.2

- 修复 Enter 键在按钮/链接上触发全屏的键盘导航问题
- 修复长按右键切换窗口后播放速度卡住的问题
- 修复视频元素为空时的崩溃问题
- 修复菜单修改设置后不立即生效的问题
- 修复快速切换页面时的初始化竞态问题
- 优化长按判定逻辑，改用时间戳避免键盘重复率影响
- 优化内存管理，使用 WeakMap 防止视频元素泄漏
- 优化 DOM 扫描性能，限制遍历深度并增量检测新视频
- 优化 Toast 提示为单例模式，避免高频调用闪烁
- 支持长按上/下箭头连续调节音量

### v1.6.1

- 修复 URL 监听器重复注册导致的性能问题
- 新增日志级别控制功能
