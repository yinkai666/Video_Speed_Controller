...
#### 新增功能
- **递归iframe域名扫描**：支持任意深度嵌套的iframe结构，包括shadow DOM和跨域iframe
- **类型安全检查**：全面添加类型验证，防止存储损坏导致的脚本崩溃
- **批量网站管理**：新增"清除所有临时启用的网站"功能，支持一键清空
- **内存优化**：修复popstate监听器内存泄漏问题，长期使用更稳定
- **智能域名识别**：启用脚本时自动扫描并启用页面中所有iframe的域名

#### 修复的Bug
1. **嵌套视频跨域问题** - 解决在iframe中嵌入视频时需要点击两次才能启用的问题
2. **无法删除临时网站** - 修复临时启用网站无法从列表中移除的问题
3. **iframe域名扫描** - 从只支持1层iframe升级到支持无限层嵌套
4. **内存泄漏** - 修复URL变化监听器未正确清理导致的内存泄漏
5. **类型安全** - 添加全面的类型检查，避免存储错误导致的崩溃
6. **历史API拦截** - 改进代码健壮性，确保正确处理this上下文
...
#### 技术改进
- 使用递归算法扫描所有层级的iframe
- 添加loadSetting()函数进行安全的类型转换
- 实现完整的监听器清理机制
- 增强错误处理和恢复能力

#### 使用建议
- 跨域iframe视频现在只需点击一次即可启用
- 建议定期使用"清除所有临时启用的网站"功能清理不需要的域名
- 如遇兼容性问题，使用"重新扫描"功能
